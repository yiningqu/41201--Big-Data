...
#repshare
sharetopics.cv <- cv.gamlr(tpcs$omega, congress109Ideology[,"repshare"])
sharewords.cv <- cv.gamlr(x, congress109Ideology[,"repshare"])
par(mfrow=c(2,2))

plot(regtopics.cv, main="topic regression of party")
plot(regwords.cv, main="phrase count regression of party")
plot(sharetopics.cv, main="topic regression of repshare")
plot(sharewords.cv, main="phrase count regression of repshare")

# max OOS R^2s
max(1-regtopics.cv$cvm/regtopics.cv$cvm[1])
max(1-regwords.cv$cvm/regwords.cv$cvm[1])
#repshare
max(1-sharetopics.cv$cvm/sharetopics.cv$cvm[1])
max(1-sharewords.cv$cvm/sharewords.cv$cvm[1])
