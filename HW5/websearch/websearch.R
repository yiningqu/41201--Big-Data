...
# 因为太大了所以不能全都plot，所以只focus on LA Times

## consider the neighborhood of latimes.com landing page

## just one link away (Order 1)

latimes <- graph.neighborhood(calink, 1, V(calink)["http://www.latimes.com/HOME/"],mode="in")[[1]]

V(latimes)$color <- "gold"

V(latimes)["http://www.latimes.com/HOME/"]$color <- "red"

plot(latimes,  vertex.frame.color=0, edge.arrow.width=.75)

## two links away (Order 2)
