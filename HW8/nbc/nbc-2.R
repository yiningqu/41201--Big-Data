...
plot(GRP ~ PE, data=shows, bg=c(4,2,3)[as.factor(shows$Genre)], pch=21, log="y")

legend("bottomright", legend=levels(as.factor(shows$Genre)), fill=c(4,2,3), bty="n")
...
o<-order(rownames(shows))

shows<-shows[o,]

o<-order(rownames(Xpilot))
...
text(zpilot[,1:2], labels=rownames(zpilot), 
    col=c("navy","red","green")[as.factor(shows$Genre)], # color by genre
    cex=shows$PE/mean(shows$PE)) # size by show


plot(zpilot[,3:4], col=0,main="shows sized by PE") 

text(zpilot[,3:4], labels=rownames(zpilot), 
    col=c("navy","red","green")[as.factor(shows$Genre)],           cex=shows$PE/mean(shows$PE)) # size by show
