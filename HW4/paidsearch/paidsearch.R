...
## plot the revenues by DMA, sample just a few

index<-sample(1:nlevels(sem$dma),20)
...
for (i in index){
	...
	s<-subset(sem,sem$dma==as.numeric(levels(sem$dma))[i])

	if (first==1){ 
		
		plot(asdate, log(s$revenue),type="l",col=s[,4]+1,ylim=log(c(min(sem$revenue),max(sem$revenue))),ylab="log revenue",main="20 randomly selected DMA's")
		
		first<-0
		
		} else{

		points(asdate, log(s$revenue),type="l",col=1-s[,4]+1)
...
semreg <- glm(y ~ dma + d*t, data=semavg)
